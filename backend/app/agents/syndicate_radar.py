    ...
    def _detect_cover_pricing(self, awards: List[Dict]) -> List[Dict]:
        ...
        for tender_id, participants in tender_groups.items():
            ...
            if highest > 0 and lowest > 0:
                # Cover pricing: second highest is very close to highest (same rate book)
                if len(sorted_amounts) >= 3:
                    gap = (sorted_amounts[-2] - lowest) / lowest
                    if gap < 0.02:  # < 2% gap between lowest and second lowest = cover price
                        findings.append({
                            ...
                            "lowest_bid": lowest,
                            "second_lowest": sorted_amounts[-2] if len(sorted_amounts) >= 2 else 0,
                            "highest_bid": highest,
                            "analysis": f"Bids tightly clustered (gap: {gap:.1%}) — possible cover pricing",
                        })
